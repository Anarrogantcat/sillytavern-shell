...
const c1 = buildProfile({ regex_scripts: [
    { scriptName: '状态栏', findRegex: '<StatusPlaceHolderImpl/>', placement: [2] },
    { scriptName: '去变量更新', findRegex: '/<UpdateVariable>.*?</UpdateVariable>/gms', placement: [2] },
]});
...
const c2 = buildProfile({ regex_scripts: [{ scriptName: '三年的水/状态栏', findRegex: '<StatusBar>([\\s\\S]*?)<\\/StatusBar>', placement: [2] }] });
...
const c3 = buildProfile({ regex_scripts: [
    { scriptName: 'AI隐藏状态栏', findRegex: '<StatusBar>[\\s\\S]*?<\\/StatusBar>', placement: [2] },
    { scriptName: '状态栏', findRegex: '<StatusBar>([\\s\\S]*?)<\\/StatusBar>', placement: [2] },
]});
check('hideTargets 含 StatusBar', c3.hideTargets.includes('StatusBar'), c3);
const r3 = guardText('正文。', c3, { injectAnchor: true, anchorStyle: 'pair' });
check('隐藏类锚点不注入', !r3.text.includes('<StatusBar>') && !r3.actions.some(a => a.type === 'anchor-injected'), r3);
