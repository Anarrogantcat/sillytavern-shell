...
- 第三位（patch）**没有上限**（`0.5.9`、`0.5.20`、`2.2.12` 都合法）；**唯一例外**：第二位正好是 36 时，第三位最多到 6
- 第二位（minor）**最多到 36**：`X.0` … `X.36`
- 加功能 / 能力增强 → 第二位 +1、第三位归零：`2.2.12` → `2.3.0`；修 bug / 优化（不新增能力）→ 第三位 +1：`2.2.12` → `2.2.13`
- 到达封顶点 `X.36.6` 后第三位不能再动，只能升第一位：`X.36.6` → `(X+1).0.0`
- 完整序列：`2.2.0 … 2.2.N` → `2.3.0 … 2.3.N` → … → `2.36.0 … 2.36.6` → `3.0.0`（**当前版本一律看 `package.json` 的 `version`，本文不写死数字**；另外 `2.0.6`、`1.36.6` 这种「满 6 进位」是旧规则的产物，现行规则下 patch 不需要因为到 6 而进位）
