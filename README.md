...
4. **回滚/指定版本**：同组里填 tag 或 commit（如 `v1.36.4`）→ 「按指定版本应用」= 从该引用取清单并**强制覆盖**（允许降级）。普通「检查扩展在线更新」永远只升不降（保护你手改过的版本）
5. **结构块严格校验**：card-compat 自带 `vendor/js-yaml.min.js`（MIT），生成后真解析角色卡的结构块；失败在面板「YAML 严格」与日志 `yaml-strict-fail` 报警（只报告不改文本），面板有「严格校验当前楼层」按钮手动复核
