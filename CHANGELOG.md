...
### 说明
- 署名（作者：小肥鱼（AI）· 染喵）属于元数据、**不算功能更新**：已并入下方 v2.1.0（套壳作者行）与 card-compat [0.3.1] / plot-pilot [0.1.3] 条目，不再单独占版本号；先前误发的 `v2.1.1` 小节已删除（tag/Release 已发布，保留在 git 历史里）
...
- **署名（元数据，不单独占版本）**：设置面板「套壳版本」下方新增一行 **作者：小肥鱼（AI）· 染喵**；`package.json` 增加 `author` 字段（electron-builder 会写进安装包的文件属性）
