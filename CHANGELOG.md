...
## v2.1.1 (2026-09-20) — 署名：小肥鱼（AI）· 染喵

### 变更
- **套壳**：设置面板「套壳版本」下方新增一行 **作者：小肥鱼（AI）· 染喵**；`package.json` 增加 `author` 字段（electron-builder 会把它写进安装包的文件属性）
- **card-compat 0.3.2 / plot-pilot 0.1.4**：扩展信息块与 `manifest.json` 的作者由 `sillytavern-shell` 改为 **小肥鱼（AI）· 染喵**（英文行同步）
- `i18n.js` 补 3 条作者相关英文对照（`作者` / 署名串 / 署名+许可整行）
- 夹具不变（compat 133 · plot-pilot 56 全绿），只改元数据
