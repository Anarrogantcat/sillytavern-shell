...
- **版本号进位规则（用户 2026-09-20 下达，已写入全局 `AGENTS.md` §5.2.1 与本 README）**：
  第三位（patch）最多到 6，满 6 再改 → 第二位 +1、第三位归零（`2.0.6` → `2.1.0`）；第二位最多到 **3**，满 3 再改 → 第一位 +1、其余归零（`2.3.6` → `3.0.0`）；
  完整序列：`2.0.0…2.0.6` → `2.1.0…2.1.6` → `2.2.0…2.2.6` → `2.3.0…2.3.6` → `3.0.0…`；
  **当前 `1.36.x` 线例外**：走完 `1.36.6` 直接进 `2.0.0`（不经过 `1.37.0`）。
  （本条 v1.36.1 发布后按用户澄清修订过一次，代码未变，故未新起版本号）
