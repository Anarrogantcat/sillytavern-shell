...
`);});
$('#btn-check-shell-update')?.addEventListener('click',async()=>{const s=$('#shell-update-status');if(!s)return;s.textContent='检查中...';s.className='update-status info';const cur=await A?.getShellVersion();const SU=window.electronAPI?.shellUpdate;if(!SU){s.textContent='自动更新不可用';s.className='update-status error';return;}try{const r=await SU.check();if(r?.hasUpdate&&r.version!==cur){s.innerHTML=`发现新版本 <b>v${r.version}</b> (当前 v${cur})`;s.className='update-status success';let dl=$('#btn-dl-shell');if(!dl){dl=document.createElement('button');dl.id='btn-dl-shell';dl.className='btn-primary';dl.style.marginTop='6px';dl.textContent='下载并安装';dl.addEventListener('click',async()=>{dl.disabled=true;dl.textContent='下载中...';s.innerHTML='下载中...';s.className='update-status info';let cleanup=SU.onProgress(({percent})=>{dl.textContent=`下载中 ${Math.round(percent)}%`;});let dc=SU.onDownloaded(()=>{cleanup();dc();s.innerHTML='✅ 更新已下载！';s.className='update-status success';dl.textContent='安装并重启';dl.disabled=false;dl.addEventListener('click',()=>SU.install(),{once:true});});let ec=SU.onError(e=>{cleanup();dc();ec();s.textContent='下载失败: '+e;s.className='update-status error';dl.disabled=false;dl.textContent='重试';});try{await SU.download();}catch(e){cleanup();dc();ec();s.textContent='下载失败: '+e.message;s.className='update-status error';dl.disabled=false;dl.textContent='重试';}});s.appendChild(dl);}}else if(r?.error){s.textContent='检查失败: '+r.error;s.className='update-status error';}else{s.textContent='已是最新版本 (v'+cur+')';s.className='update-status info';}}catch(e){s.textContent='检查失败: '+e.message;s.className='update-status error';}});
