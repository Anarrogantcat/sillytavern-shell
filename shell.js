...
$('#btn-settings-save')?.addEventListener('click',async()=>{const sp=$('#setting-server-path').value.trim();const w=parseInt($('#setting-width').value)||1280;const h=parseInt($('#setting-height').value)||800;const cb=$('#setting-close-behavior')?.value||'ask';await ST?.save({serverPath:sp,windowWidth:w,windowHeight:h,closeBehavior:cb});closeSettings();});
...
// Integrity check
$('#btn-check-integrity')?.addEventListener('click',async()=>{const s=$('#integrity-status');if(!s)return;s.textContent='检测中...';s.className='update-status info';try{const isGit=await T?.exec("git rev-parse --is-inside-work-tree 2>nul");if(isGit?.stdout?.trim()==='true'){const r1=await T?.exec("git ls-files --deleted 2>nul");const r2=await T?.exec("node -e \"console.log(require('fs').existsSync('node_modules')?'✅ node_modules':'❌ node_modules (缺失)')\"");const lines=[];if(r1?.stdout)r1.stdout.trim().split('\\n').filter(l=>l&&!l.startsWith('data/')).forEach(l=>lines.push('❌ '+l));if(r2?.stdout)lines.push(r2.stdout.trim());if(lines.length===0){s.textContent='✅ 所有文件完整 (已排除 data)';s.className='update-status success';}else{s.innerHTML='<pre style=margin:0;font-size:11px;line-height:1.6;max-height:200px;overflow-y:auto>缺失/异常文件：\\n'+lines.join('\\n')+'</pre>';s.className='update-status error';}}else{const r2=await T?.exec("node -e \"console.log(require('fs').existsSync('node_modules')?'✅ node_modules':'❌ node_modules (缺失)')\"");const ok=await T?.exec("node -e \"['server.js','package.json','public/index.html'].forEach(f=>{if(!require('fs').existsSync(f))console.log('❌ '+f)})\"");const lines=[];if(ok?.stdout)lines.push(ok.stdout.trim());if(r2?.stdout)lines.push(r2.stdout.trim());if(lines.length===0){s.textContent='✅ 核心文件完整 (非 git 安装)';s.className='update-status success';}else{s.innerHTML='<pre style=margin:0;font-size:11px;line-height:1.6;max-height:200px;overflow-y:auto>缺失文件：\\n'+lines.join('\\n')+'</pre>';s.className='update-status error';}}}catch(e){s.textContent='检测失败: '+e.message;s.className='update-status error';}});
...
`);});
$('#btn-check-shell-update')?.addEventListener('click',async()=>{const s=$('#shell-update-status');if(!s)return;s.textContent='检查中...';s.className='update-status info';const cur=await A?.getShellVersion();const SU=window.electronAPI?.shellUpdate;if(!SU){s.textContent='自动更新不可用';s.className='update-status error';return;}try{const r=await SU.check();if(r?.hasUpdate&&r.version!==cur){s.innerHTML=`发现新版本 <b>v${r.version}</b> (当前 v${cur})`;s.className='update-status success';let dl=$('#btn-dl-shell');if(!dl){dl=document.createElement('button');dl.id='btn-dl-shell';dl.className='btn-primary';dl.style.marginTop='6px';dl.textContent='下载并安装';dl.addEventListener('click',async()=>{dl.disabled=true;dl.textContent='下载中...';s.innerHTML='下载中...';s.className='update-status info';let cleanup=SU.onProgress(({percent})=>{dl.textContent=`下载中 ${Math.round(percent)}%`;});let dc=SU.onDownloaded(()=>{cleanup();dc();s.innerHTML='✅ 更新已下载！';s.className='update-status success';dl.textContent='安装并重启';dl.disabled=false;dl.addEventListener('click',()=>SU.install(),{once:true});});let ec=SU.onError(e=>{cleanup();dc();ec();s.textContent='下载失败: '+e;s.className='update-status error';dl.disabled=false;dl.textContent='重试';});try{await SU.download();}catch(e){cleanup();dc();ec();s.textContent='下载失败: '+e;s.className='update-status error';dl.disabled=false;dl.textContent='重试';}});s.appendChild(dl);}}else if(r?.error){s.textContent='检查失败: '+r.error;s.className='update-status error';}else{s.textContent='已是最新版本 (v'+cur+')';s.className='update-status info';}}catch(e){s.textContent='检查失败: '+e.message;s.className='update-status error';}});
