...
async function renderTools() {
    ...
    // 回滚列表：只列正式版（v2.1.1 起带署名）。更早的未署名旧版既不显示、也不再提供展开入口 ——
    // 那些 Release 已经在 GitHub 转成 draft，公众本来就下不到；本地若还存着旧包，直接在 lib/tools-app.js 的过滤器里忽略
    async function renderRollbackPackages() {
        const r = (await TL().rollbackList({})) || { rows: [], hidden: 0 };
        const rows = r.rows || [];
        const hid = r.hidden || 0;
        setNote(tEl.rollbackInfo, rows.length
            ? ('可用 ' + rows.length + ' 个正式版回滚包' + (hid ? '（已忽略 ' + hid + ' 个未署名旧版）' : ''))
            : (hid ? ('没有正式版回滚包（已忽略 ' + hid + ' 个未署名旧版）') : '无回滚包'));
