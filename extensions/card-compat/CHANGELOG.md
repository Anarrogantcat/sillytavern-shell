...
## [0.3.3] - 2026-09-20
### 变更
- **署名更正**：作者统一写作 **小肥鱼（DeepSeek V4 Flash）· 染喵**（此前的署名写作「小肥鱼（AI）」）；元数据更正，仍并在本版本内，不新增版本号
- **扩展信息卡改为折叠按钮**（参考「酒馆助手」信息卡的位置与样式）：面板底部只留一行 **ⓘ 扩展信息**，点一下才展开名称 / Ver / 查看日志 / 作者 / 许可 / 风险说明；展开状态记进设置（`infoOpen`），下次打开保持原状
### 说明
- 0.3.2 只改了署名（作者改为 **小肥鱼（DeepSeek V4 Flash）· 染喵**），**不算功能更新**，已并入下面的 [0.3.1] 条目，不再单独占一个版本号

## [0.3.1] - 2026-09-20
### 变更
- **署名**：扩展信息块与 manifest 的作者改为 **小肥鱼（DeepSeek V4 Flash）· 染喵**（原为 sillytavern-shell）；英文行同步为 `Author: 小肥鱼 (DeepSeek V4 Flash) & 染喵`
### 新增
