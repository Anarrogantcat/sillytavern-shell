...
const DEFAULTS = {
    ...
    // 0.31.0：变量写入策略。auto=MVU 在场就让位（默认，原始设计）；always=总是本扩展兜底写；never=从不自动写（只诊断）
    writeMode: 'auto',
...
/**
 * 0.26.0：渲染体检 —— 「卡的正则明明开着却不生效」这类问题，只靠文件看不出来，必须读运行时状态。
 * 逐条对照 ST 的 getRegexedString（regex/engine.js:334-380）与 getScriptsByType（:108-133）的实际判定条件。
 */
function renderHealth() {
    ...
    try {
        ...
        const chid = (ctx && (ctx.characterId != null ? ctx.characterId : ctx.this_chid));
        const ch = (ctx && ctx.characters && ctx.characters[chid]) || {};
        const avatar = String(ch.avatar || '');
        const allow = Array.isArray(es.character_allowed_regex) ? es.character_allowed_regex : null;
        const scripts = (ch.data && ch.data.extensions && ch.data.extensions.regex_scripts) || [];
        out.push('2) 卡内脚本: ' + (scripts.length ? scripts.map((s) => String(s.scriptName || '?') + (s.disabled ? '(停用)' : '')).join(' | ') : '（读不到）'));
        out.push('3) 当前卡 avatar: ' + JSON.stringify(avatar));
        const hit = allow ? allow.indexOf(avatar) >= 0 : false;
        out.push('4) 卡内正则在允许列表(character_allowed_regex): ' + (allow ? (hit ? '在（条件满足）' : '不在 ← 问题在这') : '（没有这个列表）'));
        if (allow && !hit) {
            const near = allow.filter((x) => String(x).indexOf(String(avatar).slice(0, 5)) >= 0).slice(0, 3);
        ...
        try {
            ...
            nodes.slice(-3).forEach((n, i) => {
                const h = String(n.innerHTML || '');
                out.push('6.' + (i + 1) + ') 近第' + (i + 1) + '条消息 DOM: 裸露锚点=' + (h.indexOf('StatusPlaceHolderImpl') >= 0) + ' 裸露UpdateVariable=' + (h.indexOf('UpdateVariable') >= 0) + ' 已画出状态栏=' + (h.indexOf('wt-outfit') >= 0 || h.indexOf('wt-location') >= 0));
            });
    ...
    try {
        ...
        for (const pool of pools) {
            ...
            if (typeof mode === 'string' && mode.indexOf('额外模型解析') >= 0) {
                ...
                // 0.31.0（P0 回归修复）：恢复原始底线 —— **MVU 在场（模式=额外模型解析）就让位**。
                // 0.26.1 我改成「自动请求关着就不让位」，结果本扩展开始写 stat_data、覆盖 MVU 的结果；
                // 模型的补丁本身残缺，用我的重放覆盖真值 → 用户实测「数据完全不更新 / 越更新越烂」。
                // 自动请求是否开着，只用于**提示**，不再用于决定是否接管（由 writeMode 决定）。
                const wm = String(((settings() || {}).writeMode) || 'auto');
                if (wm === 'never') return true;
                if (wm === 'always') return false;
                return true;   // auto：MVU 在场 → 让位（不自动写）
            }
...
/**
 * 0.25.0：判断某楼的 mes 是不是**被我们的清理逻辑砍过**。
 * 判据是硬的：拿 swipe 备档的原文跑一遍「当前规则」的清理，**砍完正好等于 mes** → 说明 mes 就是清理产物。
 * 这样才敢恢复 —— 不是猜「大概被删了」，而是能复现出同一个结果。
 */
function strippedFromSwipe(id) {
    try {
        const m = chat && chat[id];
        if (!m || m.is_user || typeof m.mes !== 'string' || !Array.isArray(m.swipes)) return null;
        const cur = String(m.mes);
        const profile = profileOf();
        const declared = new Set([...(profile.anchors || []), ...(profile.dataTags || []), ...(profile.hideTargets || []), ...(profile.strippers || []), ...(profile.rawTags || [])]);
        const o = { declared: declared, keep: KEEP_BLOCKS, bookTitles: profile.bookTitles || [], knownFields: (profile.required || []).map((x) => (x && x.path) || '') };
        for (let i = 0; i < m.swipes.length; i++) {
            const sw = String(m.swipes[i] || '');
            if (!sw || sw.length <= cur.length) continue;
            const r = stripUndeclaredBlocks(sw, o);
            if (r.removed && r.removed.length && String(r.text).trim() === cur.trim()) {
                return { index: i, text: sw, removed: r.removed, tags: r.removed.map((x) => x.tag), chars: sw.length - cur.length };
            }
        }
    } catch (e) { log('restore-check-failed', '', String((e && e.message) || e)); }
    return null;
}
/** 把被清理砍掉的原文从 swipe 备档恢复回来（写前确认、可撤销） */
async function restoreFromSwipe(id) {
    try {
        const hit = strippedFromSwipe(id);
        if (!hit) { toast(T('restoreNone'), 'info'); return false; }
        const before = String((chat[id] || {}).mes || '');
        const ok = await callGenericPopup('<b>' + escHtml(T('restoreBtn')) + '</b><br>' + escHtml(T('restoreConfirm', { tags: hit.tags.join('、'), n: hit.chars })), POPUP_TYPE.CONFIRM);
        if (!ok) return false;
        chat[id].mes = hit.text;
        try { saveChatDebounced(); } catch (_) {}
        rerenderFloor(id);
        moneyFixStack.push({ at: Date.now(), floor: id, path: T('restoreBtn'), kind: 'text', before: before });
        while (moneyFixStack.length > 20) moneyFixStack.shift();
        stats.restored = (stats.restored || 0) + 1;
        log('restore-from-swipe', '第' + id + '层', '从 swipe 恢复 ' + hit.chars + ' 字（被砍的块：' + hit.tags.join('、') + '）');
        renderStats();
        toast(T('restoreOk', { n: hit.chars }), 'success');
        return true;
    } catch (e) { log('restore-failed', '', String((e && e.message) || e)); return false; }
}
/**
 * 0.28.0：最近 N 楼里，**卡片要求但模型从没写过**的字段。
...
function neverWrittenFields(floors) {
    try {
        ...
        const n = Number(floors) > 0 ? Number(floors) : 10;
        const start = Math.max(0, chat.length - n * 2);
...
function restoreTarget() {
    try {
        ...
        const lo = Math.max(0, chat.length - 6);
        for (let i = chat.length - 1; i >= lo; i--) { const h = strippedFromSwipe(i); if (h) return { id: i, chars: h.chars }; }
    } catch (_) {}
    return null;
}
/** 应用一条资金纠正（返回是否写入） */
...
async function applyTableWrite(messageId) {
    try {
        ...
        const skip = []; const coerce = [];
        const merged = mergeStatusTable(before, tbl.data, { skipped: skip, coerced: coerce });
        ...
        const warn = [];
        if (coerce.length) warn.push(escHtml(T('tableCoerced', { n: coerce.length })) + '：' + coerce.slice(0, 3).map((c) => escHtml(c.path)).join('、'));
        if (skip.length) {
            warn.push('<b>' + escHtml(T('tableSkipped', { n: skip.length })) + '</b>');
            skip.slice(0, 5).forEach((x) => warn.push(escHtml(x.path + '：' + x.reason)));
        }
        const ok = await callGenericPopup('<b>' + escHtml(T('tableWriteBtn')) + '</b><br>' + escHtml(T('tableWriteConfirm', { n: diff.length }))
            + (warn.length ? ('<br><br>' + warn.join('<br>')) : '')
            + '<br><br>' + head + more, POPUP_TYPE.CONFIRM);
        if (!ok) return false;
        ...
        log('table-write', '第' + messageId + '层', '按状态表写回 ' + diff.length + ' 处变化（可撤销）' + (skip.length ? ('；跳过 ' + skip.length + ' 处类型不安全写入') : '') + (coerce.length ? ('；按旧结构还原 ' + coerce.length + ' 处') : ''));
        if (skip.length) log('table-write-skipped', skip.length + ' 处', skip.slice(0, 4).map((x) => x.path + '：' + x.reason).join(' ｜ '));
...
function undoMoneyFix() {
    ...
    // 0.25.0：栈里可能是「文本恢复」（从 swipe 找回被误删的块）——那条恢复的是 mes，不是变量
    if (last.kind === 'text') {
        try {
            if (chat && chat[last.floor]) { chat[last.floor].mes = last.before; try { saveChatDebounced(); } catch (_) {} }
            moneyFixStack.pop();
            log('restore-undo', '第' + last.floor + '层', '已撤销文本恢复');
        ...
        try {
            if ((settings() || {}).tableWrite !== false && !/<JSONPatch\b/i.test(m.mes)) {
                ...
                if (tblFb && tblFb.ok) {
                    ...
                    const skip0 = []; const coerce0 = [];
                    const merged = mergeStatusTable(base || {}, tblFb.data, { skipped: skip0, coerced: coerce0 });
                    const diff = statusTableDiff(base || {}, merged, 40);
                    report.table = { tag: tblFb.tag, changes: diff.length, sample: diff.slice(0, 6), base: !!base, fromSwipe: !(tbl && tbl.ok), skipped: skip0.length, coerced: coerce0.length, skipSample: skip0.slice(0, 4) };
                }
            }
        } catch (_) {}
...
function ensureVarBar() {
    ...
    bar.appendChild(b);
    // 0.24.0：把「按状态表写回」也放到发送栏 —— 它才是这条链路上真正需要动手的那个按钮，
    // 埋在面板第③组里用户找不到（用户实测反馈）。仅在该楼确实有可写回的状态表时才显示，不做死按钮。
    const bt = document.createElement('div');
    bt.id = 'cc-var-table';
    bt.className = 'ccv-btn ccv-table';
    bt.textContent = T('varTableBtn');
    bt.style.display = 'none';
    bt.addEventListener('click', async () => {
        try {
            const tg = tableWriteTarget();
            if (!tg) { toast(T('tableNoChange'), 'info'); return; }
            await applyTableWrite(tg.id);
            refreshTableButton();
        } catch (e) {
            log('table-btn-failed', '', String((e && e.message) || e));
            toast(T('moneyFixWriteFail') + String((e && e.message) || e), 'warning');
        }
    });
    bar.appendChild(bt);
    // 0.25.0：第三个按钮 —— 只在本楼「确实被我们的清理砍过、且能从 swipe 复原」时出现
    const br = document.createElement('div');
    br.id = 'cc-var-restore';
    br.className = 'ccv-btn ccv-restore';
    br.textContent = T('restoreBtn');
    br.style.display = 'none';
    br.addEventListener('click', async () => {
        try {
            const tg = restoreTarget();
            if (!tg) { toast(T('restoreNone'), 'info'); return; }
            await restoreFromSwipe(tg.id);
            refreshTableButton();
        } catch (e) { log('restore-btn-failed', '', String((e && e.message) || e)); }
    });
    bar.appendChild(br);
    // 0.26.0：渲染体检按钮（放在发送栏，与其它按钮并排）
    const bh = document.createElement('div');
    bh.id = 'cc-var-health';
    bh.className = 'ccv-btn ccv-health';
    bh.textContent = T('healthBtn');
    bh.addEventListener('click', async () => {
        try {
            const txt = renderHealth();
            log('render-health', '', txt.replace(/\n/g, ' ｜ '));
            try { await navigator.clipboard.writeText(txt); toast(T('healthCopied'), 'success'); } catch (_) { toast(T('healthCopied'), 'success'); }
            await callGenericPopup('<b>' + escHtml(T('healthBtn')) + '</b><br><span class="cc-log-doc">' + escHtml(txt).replace(/\n/g, '<br>') + '</span>', POPUP_TYPE.TEXT);
        } catch (e) { log('health-failed', '', String((e && e.message) || e)); }
    });
    bar.appendChild(bh);
    form.prepend(bar);
...
function tableWriteTarget() {
    try {
        if (!chat || !chat.length) return null;
        const lo = Math.max(0, chat.length - 6);
        for (let i = chat.length - 1; i >= lo; i--) {
            const m = chat[i];
            if (!m || m.is_user || typeof m.mes !== 'string') continue;
            const t0 = parseStatusTable(m.mes, { yamlLib: yamlLibSync() });
            const tbl = (t0 && t0.ok) ? t0 : (Array.isArray(m.swipes) ? (m.swipes.map((x) => parseStatusTable(String(x || ''), { yamlLib: yamlLibSync() })).find((x) => x && x.ok) || null) : null);
            if (!tbl || !tbl.ok) continue;
            const base = mvuVarsOf(i) || mvuVarsOf(i - 1) || null;
            if (!base) continue;
            const merged = mergeStatusTable(base, tbl.data, { skipped: [] });
            const n = statusTableDiff(base, merged, 40).length;
            return n > 0 ? { id: i, n: n } : null;
        }
    } catch (e) { log('table-target-failed', '', String((e && e.message) || e)); }
    return null;
}
/** 发送栏那个按钮的显隐与计数（0.24.0） */
function refreshTableButton() {
    try {
        const bt = document.getElementById('cc-var-table');
        if (!bt) return;
        if (!settings() || settings().enabled === false || settings().tableWrite === false) { bt.style.display = 'none'; return; }
        const tg = tableWriteTarget();
        if (!tg) { bt.style.display = 'none'; } else { bt.textContent = T('varTableBtn') + '（' + tg.n + '）'; bt.style.display = ''; }
        const br = document.getElementById('cc-var-restore');
        if (br) {
            const rg = restoreTarget();
            if (rg) { br.textContent = T('restoreBtn') + '（' + rg.chars + ' 字）'; br.style.display = ''; } else br.style.display = 'none';
        }
    } catch (_) {}
}
...
function renderCoverageTable() {
    ...
    try {
        const mp = neverWrittenFields(12);
        if (mp.length) parts.push('<div class="cc-line cc-warn">' + escHtml(T('neverWritten', { n: mp.length, list: mp.slice(0, 6).join('、') })) + '</div>');
    } catch (_) {}
    if (rep.table && rep.table.changes > 0 && rep.table.base) {
        ...
        const extra = [];
        if (tbl.coerced) extra.push(escHtml(T('tableCoerced', { n: tbl.coerced })));
        if (tbl.skipped) {
            extra.push(escHtml(T('tableSkipped', { n: tbl.skipped })));
            (tbl.skipSample || []).forEach((x) => extra.push(escHtml(x.path + '：' + x.reason)));
        }
        parts.push('<div class="cc-line cc-warn"><b>' + escHtml(T('tableWarn')) + '</b>：' + escHtml(T('tableWarnBody', { tag: tbl.tag, n: tbl.changes })) + '<br>' + head + '<br>'
            + (extra.length ? ('<span class="cc-muted">' + extra.join('<br>') + '</span>') : '')
...
function buildSettingsUi() {
    ...
    wrap.innerHTML = [
        ...
        cb('cc-var-auto', 'varAuto'), cb('cc-var-repair', 'varRepair'), cb('cc-overdraft', 'overdraftGuard'), cb('cc-schema-guard', 'schemaGuard'), cb('cc-var-barcb', 'varBar'), cb('cc-money-check', 'moneyCheck'), cb('cc-money-fix', 'moneyFix'),
            '<label class="cc-line cc-muted">' + escHtml(T('writeMode')) + ' <select id="cc-write-mode"><option value="auto">' + escHtml(T('writeModeAuto')) + '</option><option value="always">' + escHtml(T('writeModeAlways')) + '</option><option value="never">' + escHtml(T('writeModeNever')) + '</option></select></label>', cb('cc-table-write', 'tableWrite'),
...
'<details class="cc-grp"><summary>⑥ ' + escHtml(T('secScan')) + '</summary>',
    ...
    try {
        const wmsel = document.getElementById('cc-write-mode');
        if (wmsel) {
            wmsel.value = String((settings() || {}).writeMode || 'auto');
            wmsel.addEventListener('change', () => { settings().writeMode = wmsel.value; saveSettingsDebounced(); renderStats(); log('write-mode', wmsel.value, '变量写入策略已切换'); });
        }
    } catch (_) {}
