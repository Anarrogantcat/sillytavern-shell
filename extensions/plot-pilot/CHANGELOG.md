...
### 变更
- **把 0.2.0 的「不再重复发送 / 不再谎报」做成可验证的纯函数**：新增 `getFailureFallback(mode, ctx)` + `fallbackReason(mode, ctx)`，`send()` 的失败分支改为调用它们 —— 逻辑可测，不再依赖真实 DOM
- **补齐夹具**（原先 DOM 行为零覆盖）：新增 10 条断言 —— api 模式不回退、输入框已空不回退、auto + 有内容才回退、dom 不可用不回退、回退原因可读；并**拿真实 `sendViaDom` 实现**注入假 DOM 跑通「按钮一直不可用 → 抛错且不点击」「点了但输入框没变化 → 判失败」两条路径

## [0.2.1] - 2026-09-25

### 变更
- 仅版本号：**本版本无功能性代码改动**。`send()` 的失败回退与 `sendViaDom` 的超时判定尚无自动夹具（依赖 DOM/ST，属已知覆盖盲区，已记录在 `docs/extension-roadmap.md`）

## [0.2.0] - 2026-09-25

### 修复
- **API 模式失败后仍会模拟点击 → 可能把同一句话发两遍**：`send()` 的兜底原先无条件走 `sendViaDom`。现在只有 `sendMode='auto'` 才回退；`sendMode='api'`（用户明确要求只用 API）不再改成模拟点击；回退前还会检查输入框是否已空（API 可能已经发出去了）并跳过，避免重复
- **DOM 超时后谎报「已发送」**：`sendViaDom` 原先在 deadline 到点后无条件 `btn.click()`，调用方照样记 `sent` —— 按钮一直禁用时点击无效，面板却显示成功。现在超时直接抛错（不记 sent），点击后再确认输入框确实被清空，没清空也判失败
- **配置写入绕过 sanitizeConfig**：`bindText` 原先直接把 `el.value`（输入框没有 maxlength）存进设置。现在写入前按 2000 字（与 sanitizeConfig 上限一致）截断

## [0.1.5] - 2026-09-20
### 变更
- **扩展信息卡改为折叠按钮**：面板底部只留一行 **ⓘ 扩展信息**，点一下才展开名称 / Ver / 查看日志 / 作者 / 许可 / 风险说明；展开状态记进设置（`infoOpen`）

## [0.1.3] - 2026-09-20
### 新增
- **扩展信息块**：名称 + `Ver 0.1.3` + 作者 / 许可 / 项目主页 + 免费使用与风险说明（本扩展只往输入框发指令，不改消息、不改变量）
