...
export function registerAppTools(ctx) {
    ...
    // 主进程文案翻译：由 index.js 注入（复用 i18n.js 同一份字典）；未注入时原样返回
    const tr = typeof ctx.t === 'function' ? ctx.t : (s => s);
    ...
    async function doBackup(note) {
        ...
        // B8 备份增强：zip 压缩（审计：路径拼进 PowerShell 命令有注入风险 → 改用参数数组 spawn）
        const s0 = getSettings();
        if (s0.backupZip) {
            // 审计：原实现在 PowerShell 退出后无条件删掉未压缩目录 —— Compress-Archive 失败（路径含空格/引号、PS 不可用）时
            // 用户会得到一个「有备份名、实际是空目录」的假备份。现在先校验 zip 真的产出，再删原目录；失败则保留原目录兜底。
            const zipped = await new Promise((resolve) => {
                ...
                const done = (code) => {
                    ...
                    exited = true;
                    try {
                        const ok = code === 0 && fs.existsSync(dest + '.zip') && fs.statSync(dest + '.zip').size > 0;
                        if (ok) { try { fs.rmSync(dest, { recursive: true, force: true }); } catch (_) {} }
                        else { terminalWrite('[backup] ⚠ zip 压缩未成功（退出码 ' + code + '），已保留未压缩备份目录：' + dest + '\n'); }
                        resolve(ok);
                    } catch (_) { resolve(false); }
                };
                try {
                    const ps = spawn('powershell', ['-NoProfile', '-NonInteractive', '-Command', 'Compress-Archive', '-Path', dest, '-DestinationPath', dest + '.zip', '-CompressionLevel', 'Optimal'], { timeout: 600000, windowsHide: true });
                    ps.on('exit', (code) => done(code));
                    ps.on('error', () => done(-1));
                } catch (_) { done(-1); }
            });
            if (!zipped && !fs.existsSync(dest)) throw new Error('备份既没生成 zip 也没保留目录：' + dest);
        }
        const keep = getSettings().backupKeep || 5;
        // 只把「完整备份」计数：目录、或大小 > 0 的 zip（未压缩兜底目录与它可能同时存在，不能当两次）
        const baseOf = (n) => n.replace(/\.zip$/i, '');
        const entries = fs.readdirSync(dir);
        const all = entries.filter((n) => /^ST-Data-/.test(n) && (!/\.zip$/i.test(n) || (() => { try { return fs.statSync(path.join(dir, n)).size > 0; } catch (_) { return false; } })()))
            .map(baseOf)
            .filter((v, i, a) => a.indexOf(v) === i)
            .sort();
        while (all.length > keep) {
            const base = all.shift();
            fs.rmSync(path.join(dir, base), { recursive: true, force: true });
            try { fs.rmSync(path.join(dir, base + '.zip'), { force: true }); } catch (_) {}
        }
    ...
    function backupCheck() { // 启动时 + 每 6h 检查定时备份
        const s = getSettings();
        // 未配置备份目录时也按默认目录执行，否则界面显示“已开启”但永远不会定时备份
        if (!s.backupAuto) return;
        const last = s.backupLast || 0;
        const intervalH = s.backupIntervalH || 24;
        if (Date.now() - last > intervalH * 3600e3) {
            doBackup().catch(e => terminalWrite(`[backup] 自动备份失败: ${e.message}\n`));
        }
    }
    ...
    async function restoreBackup(name) {
        ...
        if (!safeName || safeName.includes('..')) throw new Error('无效备份名');
        const src = path.join(dir, safeName);
        if (!fs.existsSync(src)) throw new Error('备份不存在: ' + safeName);
        // 恢复前先备份当前数据，防止误操作
        await doBackup('before-restore');
        if (ctx.stopServer) { try { ctx.stopServer(); } catch (_) {} }
        const dataTmp = path.join(dir, '.data-tmp-' + Date.now());
        try {
            if (fs.existsSync(dataRoot)) fs.renameSync(dataRoot, dataTmp);
            fs.mkdirSync(dataRoot, { recursive: true });
            fs.cpSync(src, dataRoot, { recursive: true, force: true });
            terminalWrite(`[backup] 已从 ${safeName} 恢复数据\n`);
            return { ok: true, dest: dataRoot };
        } catch (e) {
            try { fs.rmSync(dataRoot, { recursive: true, force: true }); } catch (_) {}
            try { if (fs.existsSync(dataTmp)) fs.renameSync(dataTmp, dataRoot); } catch (_) {}
            throw e;
        } finally {
            try { if (fs.existsSync(dataTmp)) fs.rmSync(dataTmp, { recursive: true, force: true }); } catch (_) {}
            if (ctx.startServer) { try { await ctx.startServer(); } catch (_) {} }
        }
    }

    // ── A2 版本回滚 ────────────────────────────────────────────────
    const rollbackDir = () => path.join(app.getPath('userData'), 'rollback');
    // 正式版起点：v2.1.1 起带署名；更早的是历史预览版（默认不在回滚列表里显示）
    const SIGNED_FROM = '2.1.1';
    function cmpVer(a, b) {
    ...
    /** 回滚包列表：只给正式版（>= SIGNED_FROM）；opts.all=true 可临时带上未署名旧版（0.6.0 起界面不再提供入口，保留给排查用） */
    function rollbackList(opts = {}) {
        const dir = rollbackDir();
        if (!fs.existsSync(dir)) return { rows: [], hidden: 0, signedFrom: SIGNED_FROM };
        const all = fs.readdirSync(dir).filter(f => f.endsWith('.exe')).map(f => {
            const m = f.match(/Setup-([\d.]+)\.exe/);
            return { file: f, version: m ? m[1] : f.replace('.exe', '') };
        }).sort((a, b) => cmpVer(b.version, a.version));
        const rows = opts.all ? all : all.filter((r) => cmpVer(r.version, SIGNED_FROM) >= 0);
        return { rows, hidden: all.length - rows.length, signedFrom: SIGNED_FROM };
    }
    function saveRollbackPackage(version, installerPath) {
        try {
            const v = safeVersion(version);
            if (!v) { terminalWrite('[rollback] 跳过：版本号不合法 ' + String(version) + '\n'); return; }
            const dir = rollbackDir();
            fs.mkdirSync(dir, { recursive: true });
            const dest = path.join(dir, `SillyTavern-Setup-${v}.exe`);
            if (!fs.existsSync(dest)) { fs.copyFileSync(installerPath, dest); terminalWrite(`[rollback] 已保留 v${v} 安装包\n`); }
        } catch (_) {}
    }
    /** 版本号只允许数字与点：它是拼接进文件名的，带路径分隔符/.. 会越出 rollbackDir（审计：路径穿越 → 可执行任意 exe） */
    function safeVersion(v) { return /^[0-9]+(?:\.[0-9]+)*$/.test(String(v || '')) ? String(v) : null; }
    function rollbackInstall(version) {
        const v = safeVersion(version);
        if (!v) return { error: '非法版本号（只接受 x.y.z 形式的数字版本）' };
        const f = path.join(rollbackDir(), `SillyTavern-Setup-${v}.exe`);
        if (!fs.existsSync(f)) return { error: '回滚包不存在' };
        // 审计：应用运行中安装器覆盖自身 exe 大概率失败 → 先退出再装
        terminalWrite(`[rollback] 正在回滚到 v${version}，应用将退出...\n`);
    ...
    // ── IPC 注册 ───────────────────────────────────────────────────
    ipcMain.handle('tools:backupNow', async (e, note) => { try { return { ok: true, dest: await doBackup(note) }; } catch (err) { return { error: err.message }; } });
    ipcMain.handle('tools:backupList', () => backupList());
    ipcMain.handle('tools:backupRestore', async (e, name) => { try { return await restoreBackup(name); } catch (err) { return { error: err.message }; } });
    ...
    ipcMain.handle('tools:backupSave', (e, cfg) => {
        ...
        saveSettings(s);
        if (s.backupAuto) { doBackup().catch(() => {}); }
        return true;
    });
    ipcMain.handle('tools:rollbackList', (_e, opts) => rollbackList(opts || {}));
    ...
    ipcMain.handle('tools:portablePick', async () => {
        const r = await dialog.showOpenDialog({ properties: ['openDirectory'], title: tr('选择便携包导出位置') });
        if (r.canceled || !r.filePaths[0]) return { canceled: true };
        try { return { canceled: false, dest: await portableExport(r.filePaths[0]) }; }
        catch (e) { return { canceled: false, error: e.message }; }
    });

    return { doBackup, restoreBackup, backupList, saveRollbackPackage, rollbackList, rollbackInstall, setImmersive, getAutostart, setAutostart, verifyPin, backupCheck };
}
