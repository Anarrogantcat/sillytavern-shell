        ...
        if (s0.backupZip) {
            try {
                await new Promise((resolve) => {
                    const ps = spawn('powershell', ['-NoProfile', '-Command', 'Compress-Archive', '-Path', dest, '-DestinationPath', dest + '.zip', '-CompressionLevel', 'Optimal'], { timeout: 600000, windowsHide: true });
                    ps.on('exit', () => { try { fs.rmSync(dest, { recursive: true, force: true }); } catch (_) {} resolve(); });
                    ps.on('error', () => resolve());
                });
            } catch (_) {}
        }
