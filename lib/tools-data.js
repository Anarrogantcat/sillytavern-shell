                        ...
                        if (keyword === 'chara' || keyword === 'ccv3') {
                            ...
                            if (type === 'zTXt') {
                                try {
                                    ...
                                    text = z.inflateSync(Buffer.from(text, 'latin1')).toString('utf8');
                                } catch (_) {}
                            }
...
async function chatOnce(model, prompt, maxTokens = 512) {
    ...
    if (model.source === 'gemini') {
        const r = await fetch(`${base}/v1beta/models/${encodeURIComponent(model.model)}:generateContent?key=${encodeURIComponent(model.key || '')}`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
