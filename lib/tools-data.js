...
export function registerDataTools(ctx) {
    ...
    function notifyGenerated(charName, tokens) {
        ...
        const n = new Notification({ title: tr('💬 回复生成完成'), body: `${charName}${tr(' 的新回复已生成')}${tokens ? tr(`（约 ${tokens} token）`) : ''}` });
    ...
    async function exportCharacters() {
        ...
        const r = await dialog.showOpenDialog({ properties: ['openDirectory'], title: tr('选择角色卡导出目录') });
    ...
    function chatStats() {
        const files = scanChats();
        const out = { chats: files.length, totalMessages: 0, totalChars: 0, userChars: 0, replyChars: 0, replies: 0, userMsgs: 0, perChar: [] };
        for (const { char, file } of files) {
            let text = '';
            try { text = fs.readFileSync(file, 'utf8'); } catch (_) { continue; }
...
<body><h1>${f.char} · 共 ${msgs.length} 条消息</h1>${rows}</body></html>`;
    ...
    // ── B6 角色卡速览（解析 PNG tEXt chunk，只读）────────────────────
    // 正确按 PNG chunk 结构解析（IHDR→tEXt→IDAT→IEND），关键字 chara/ccv3，
    // 不截断 JSON（旧实现全文搜 'chara\0' 在角色卡 JSON 大时误匹配/截断）
    function extractCharaJson(buf) {
        if (!buf || buf.length < 8) return null;
        // 字节级签名比较（toString('ascii') 会把 0x89 截断成 tab 导致永远不匹配！）
        if (buf[0] !== 0x89 || buf.toString('ascii', 1, 8) !== 'PNG\r\n\x1a\n') return null;
        let off = 8;
        while (off + 12 <= buf.length) {
            ...
            if (type === 'tEXt' || type === 'zTXt') {
                try {
                    ...
                    if (sep > 0) {
                        const keyword = chunk.toString('ascii', 0, sep);
                        if (keyword === 'chara' || keyword === 'ccv3') {
                            let text = chunk.toString('utf8', sep + 1);
                            if (type === 'zTXt') {
                                try {
                                    // zTXt 布局：keyword + \0 + 1 字节压缩方式(0=zlib/deflate) + 压缩数据
                                    text = zlib.inflateSync(chunk.subarray(sep + 2)).toString('utf8');
                                } catch (_) {
                                    // 解压失败时退回原始文本，交给上层 JSON 解析继续尝试
                                    text = chunk.toString('latin1', sep + 2);
                                }
                            }
    ...
    function cardPreview(cardName) {
        // 审计：路径穿越防护（拒绝 .. 与绝对路径）
        const name = String(cardName || '').replace(/[\\/]/g, '');
        if (!name || name.includes('..')) throw new Error('无效的角色卡名');
        const file = path.join(charsRoot(), name);
        if (!fs.existsSync(file)) throw new Error('角色卡不存在: ' + name);
        let json = null;
        try {
            const buf = fs.readFileSync(file);
            const raw = extractCharaJson(buf);
            if (raw) {
                // ST 1.18 tEXt value 是 Base64 编码的 JSON（实测：'eyJ' 开头），
                // 老版本可能是 <json> 包裹或裸 JSON——双通道尝试
                const candidates = [];
                const b64 = Buffer.from(raw.trim(), 'base64').toString('utf8');
                const m1 = raw.match(/<json>([\s\S]*?)<\/json>/);
                if (m1) candidates.push(m1[1]);
                const m2 = b64.match(/<json>([\s\S]*?)<\/json>/);
                if (m2) candidates.push(m2[1]);
                candidates.push(raw, b64);
                for (const c of candidates) {
                    try { json = JSON.parse(c.trim()); if (json) break; } catch (_) {}
                }
            }
        } catch (_) { json = null; }
        if (!json) throw new Error('无法解析角色卡数据（可能不是 ST 角色卡）');
        const spec = json.spec || json.data?.spec || {};
        const first = json.data?.first_mes || json.first_mes || '';
        return {
            name: spec.name || json.name || cardName.replace(/\.\w+$/, ''),
            description: spec.description || json.description || '',
            personality: spec.personality || json.personality || '',
            scenario: spec.scenario || json.scenario || '',
            firstMes: String(first).slice(0, 300),
            mesExamples: String(json.data?.mes_example || json.mes_example || '').slice(0, 300),
        };
    }
...
function detectModel(ctx) {
    try {
        ...
        const map = {
            custom: ['custom_url', 'custom_model', 'custom_key'], openai: ['openai_url', 'openai_model', 'openai_key'],
            ollama: ['ollama_url', 'ollama_model'], openrouter: ['openrouter_url', 'openrouter_model', 'openrouter_key'],
            claude: ['claude_url', 'claude_model', 'claude_api_key'], gemini: ['google_url', 'google_model', 'google_api_key'],
        };
        const keys = map[src];
        let url = '', model = '', key = '';
        if (keys) { url = oai[keys[0]] || ''; model = oai[keys[1]] || ''; key = oai[keys[2]] || ''; }
        if (!url && oai.custom_url) { url = oai.custom_url; if (!model) model = oai.custom_model; }
        return url && model ? { url, model, source: src, key } : null;
    } catch (_) { return null; }
}
async function chatOnce(model, prompt, maxTokens = 512) {
    ...
    // 审计：云 API 直连带鉴权——Claude 用 /v1/messages + x-api-key；Gemini 用 generateContent + key；其余 OpenAI 兼容带 Bearer
    if (model.source === 'claude') {
    ...
    if (model.source === 'gemini') {
        // ST 的 google_url 默认可能已带 /v1beta，先剥离再拼接，避免 /v1beta/v1beta
        const geminiBase = base.replace(/\/v1beta\/?$/, '').replace(/\/+$/, '');
        const r = await fetch(`${geminiBase}/v1beta/models/${encodeURIComponent(model.model)}:generateContent?key=${encodeURIComponent(model.key || '')}`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
