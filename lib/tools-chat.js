...
export function registerChatTools(ctx) {
    ...
    function sessionSave(id, title, messages) {
        // 与 sessionLoad/sessionDelete 相同的 id 校验，防止路径穿越写文件
        const sid = String(id || '').replace(/[\\/]/g, '');
        if (!sid || sid.includes('..') || !/^[A-Za-z0-9_-]+$/.test(sid)) return false;
        const dir = sessionsDir();
        fs.mkdirSync(dir, { recursive: true });
        fs.writeFileSync(path.join(dir, sid + '.json'), JSON.stringify({ id: sid, title, messages, updated: Date.now() }, null, 2), 'utf8');
        return true;
    }
