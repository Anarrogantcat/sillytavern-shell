...
/**
 * 按 git 引用（tag / 分支 / commit）构造源地址 —— 回滚就是"从旧引用取那份扩展"。
 * 非法引用一律退回 main，避免把用户输入拼进 URL。
 */
export function basesForRef(ref) {
    const r = String(ref || '').trim();
    const safe = /^[A-Za-z0-9._\-\/]{1,80}$/.test(r) ? r : 'main';
    return [
        'https://raw.githubusercontent.com/Anarrogantcat/sillytavern-shell/' + safe + '/extensions',
        'https://testingcf.jsdelivr.net/gh/Anarrogantcat/sillytavern-shell@' + safe + '/extensions',
    ];
}

/**
 * 回滚/指定版本安装：从给定引用取清单，并**强制**按该引用落地（允许降级、同版本重写）。
 * @param {{ref:string, dataRoot:string, fetchText:Function, fetchBinary?:Function, log?:Function}} opts
 */
export async function rollbackTo(opts = {}) {
    const ref = String(opts.ref || '').trim();
    if (!/^[A-Za-z0-9._\-\/]{1,80}$/.test(ref)) return { ok: false, ref, summary: '版本/提交号不合法（只允许字母数字 . _ - /）', tried: [] };
    const bases = opts.bases || basesForRef(ref);   // 允许注入（单测用假源）
    const stamp = opts.stamp || Date.now();
    const got = await fetchIndex({ fetchText: opts.fetchText, bases, log: opts.log, stamp });
    if (!got.ok) return { ok: false, ref, summary: '无法从 ' + ref + ' 取到扩展清单', tried: got.tried };
    const res = await applyRemoteUpdates({
        index: got.index, base: got.base, bases, dataRoot: opts.dataRoot, force: true, stamp,
        fetchText: opts.fetchText, fetchBinary: opts.fetchBinary, log: opts.log,
    });
    return { ok: true, ref, base: got.base, index: got.index, updated: res.updated, skipped: res.skipped, failed: res.failed, summary: summarizeRemote(res) };
}
