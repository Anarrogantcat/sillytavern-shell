...
export function registerEnvTools(ctx) {
    ...
    async function ollamaServiceStatus() {
        try {
            const r = await fetch('http://localhost:11434/api/tags', { signal: AbortSignal.timeout(3000) });
            return { running: r.ok, binary: findOllamaBinary() };
        } catch (_) { return { running: false, binary: findOllamaBinary() }; }
    }
    async function ollamaStartService() {
        const bin = findOllamaBinary();
        if (!bin) return { error: '未找到 ollama.exe，请安装 Ollama' };
        try {
            const child = spawn(bin, ['serve'], { detached: true, stdio: 'ignore', windowsHide: true });
            child.unref();
            await new Promise(r => setTimeout(r, 1500));
            const st = await ollamaServiceStatus();
            return st.running ? { ok: true } : { error: '服务已启动但暂未就绪，请稍后重试' };
        } catch (e) { return { error: e.message }; }
    }
    ...
    function findLlamaServerExe() {
        const candidates = [process.env.LLAMA_SERVER_BIN || '', 'C:\\Program Files\\llama.cpp\\llama-server.exe', 'C:\\llama.cpp\\build\\bin\\Release\\llama-server.exe'].filter(Boolean);
        for (const c of candidates) { try { if (c && fs.existsSync(c)) return c; } catch (_) {} }
        return null;
    }
    async function llamaStatus(port) {
        const p = parseInt(port) || 8080;
        try {
            const r = await fetch(`http://127.0.0.1:${p}/v1/models`, { signal: AbortSignal.timeout(2500) });
            return { running: r.ok, port: p };
        } catch (_) { return { running: false, port: p }; }
    }
    async function llamaStart(cfg) {
        const bin = cfg?.bin || findLlamaServerExe();
        if (!bin) return { error: '未指定 llama-server 路径（可手动填 .exe）' };
        if (!cfg?.model) return { error: '未指定模型 .gguf 路径' };
        const args = ['-m', cfg.model, '--port', String(cfg.port || 8080), '--host', String(cfg.host || '127.0.0.1'), '--ctx-size', String(cfg.ctx || 4096), '--threads', String(cfg.threads || 0), '--n-gpu-layers', String(cfg.gpuLayers ?? 0)];
        try {
            const child = spawn(bin, args, { detached: true, stdio: 'ignore', windowsHide: true });
            child.unref();
            for (let i = 0; i < 20; i++) {
                await new Promise(r => setTimeout(r, 500));
                const st = await llamaStatus(cfg.port || 8080);
                if (st.running) return { ok: true, port: st.port, pid: child.pid };
            }
            return { error: '启动后 10s 内未就绪，请检查模型路径/参数' };
        } catch (e) { return { error: e.message }; }
    }
    async function llamaStop(port) {
        const p = parseInt(port) || 8080;
        const net = await execP(`netstat -ano | findstr :${p} | findstr LISTENING`, 5000);
        if (!net.ok || !net.out) return { ok: false, error: '未找到监听 ' + p + ' 的进程' };
        const pid = net.out.trim().split(/\s+/).pop();
        const k = await execP(`taskkill /F /PID ${pid}`, 5000);
        return k.ok ? { ok: true, pid } : { ok: false, error: k.out || '停止失败' };
    }
    ...
    async function healthCheck() {
        const checks = await envCheck();
        // ZeroTier
        const ztCandidates = ['C:\\Program Files (x86)\\ZeroTier\\One\\zerotier-cli.exe', 'C:\\Program Files\\ZeroTier\\One\\zerotier-cli.exe'];
        const ztCli = ztCandidates.find(p => fs.existsSync(p)) || null;
        if (ztCli) {
            const r = await execP(`"${ztCli}" info`);
            checks.push({ name: 'ZeroTier', ok: r.ok && /200 info/i.test(r.out), detail: r.ok ? r.out.split('\n')[0] : '未运行' });
        } else {
            checks.push({ name: 'ZeroTier', ok: false, detail: '未安装' });
        }
        // Cloudflared
        const cf = path.join(process.resourcesPath, 'cloudflared.exe');
        checks.push({ name: 'Cloudflared', ok: fs.existsSync(cf), detail: fs.existsSync(cf) ? '已随套壳分发' : '未找到（开发模式正常）' });
        // ST 白名单状态（只读 config.yaml，不修改）
        try {
            const cfgPath = path.join(ctx.sillyTavernRoot || '', 'config.yaml');
            if (fs.existsSync(cfgPath)) {
                const cfg = fs.readFileSync(cfgPath, 'utf8');
                const m = cfg.match(/^whitelistMode:\s*(\S+)/m);
                checks.push({ name: 'ST 白名单', ok: true, detail: m ? 'whitelistMode=' + m[1] : '未找到配置' });
            }
        } catch (_) {}
        return checks;
    }
    ...
    ipcMain.handle('tools:healthCheck', async () => healthCheck());
    ipcMain.handle('tools:qrcode', async (_e, text) => {
        if (typeof text !== 'string' || !text.trim()) return { error: '内容为空' };
        try { const url = await QRCode.toDataURL(text.trim(), { width: 240, margin: 1 }); return { url }; } catch (e) { return { error: e.message }; }
    });
    ...
    ipcMain.handle('tools:ollamaStatus', async () => ollamaServiceStatus());
    ipcMain.handle('tools:ollamaStart', async () => ollamaStartService());
    ipcMain.handle('tools:llamaStatus', (_e, port) => llamaStatus(port));
    ipcMain.handle('tools:llamaStart', (_e, cfg) => llamaStart(cfg));
    ipcMain.handle('tools:llamaStop', (_e, port) => llamaStop(port));
    ipcMain.handle('tools:gpuStats', async () => gpuStats());
    ipcMain.handle('tools:clashCheck', async () => checkClash());

    return { envCheck, healthCheck, ollamaModels, ollamaPs, ollamaAction, ollamaStatus, ollamaStart, llamaStatus, llamaStart, llamaStop, gpuStats, checkClash };
}
